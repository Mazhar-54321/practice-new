...
## useState hook object — detailed structure

```js
{
  memoizedState: 0,     // current state value
  baseState: 0,         // base before any updates
  queue: {
    pending: {
      action: value,    // new value or updater function
      next: →           // circular — points back to first update
    },
    dispatch: setCount  // your setter function
  },
  next: →               // points to next hook in Fiber's linked list
}
```

- `queue.pending` = circular linked list of pending updates — processed in order on next render
- `queue.dispatch` = your setter function (`setCount`)
- `next` inside queue = circular list of updates
- `next` on hook object = next hook in Fiber linked list
- Same name `next`, two different levels
